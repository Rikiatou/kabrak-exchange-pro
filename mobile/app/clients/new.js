...
export default function NewClientScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    if (result.success) {
      Alert.alert(t.common.success, t.clients.createBtn + ' ✓', [
        { text: 'Voir le client', onPress: () => router.replace(`/clients/${result.data.id}`) },
        { text: 'Nouveau client', onPress: () => setForm({ name: '', phone: '', email: '', idNumber: '', idType: 'national_id', address: '', notes: '' }) }
      ]);
    } else {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t.clients.newTitle}</Text>
          <View style={{ width: 32 }} />
        </View>
          ...
          <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={loading}>
            {loading ? <ActivityIndicator color={COLORS.white} /> : (
              <>
                ...
                <Text style={styles.submitBtnText}>{t.clients.createBtn}</Text>
              </>
            )}
          </TouchableOpacity>
